...
function roda(pergunta, projeto, flags) {
  ...
  return { srcs, bytes: saida.length };
}
...
const acc = {};
for (const [nome] of motores) acc[nome] = { recall: 0, hit3: 0, rr: 0, nos: 0 };

const linhas = [];
for (const q of golden) {
  const row = { pergunta: q.pergunta.slice(0, 40), projeto: q.projeto };
  for (const [nome, flags] of motores) {
    const { srcs } = roda(q.pergunta, q.projeto, flags);
...
const col = (m) => m ? `${m.recall ? '✓' : '·'}${m.hit3 ? '³' : ' '} r${m.rr.toFixed(2)}` : '     ';
...
for (const [nome] of motores) {
  const a = acc[nome];
  console.log(`  ${nome.padEnd(10)} recall ${a.recall}/${N} · hit@3 ${a.hit3}/${N} · MRR ${(a.rr / N).toFixed(3)} · nós/pergunta ${(a.nos / N).toFixed(1)}`);
}
